...
const OTPModal = ({ onClose }) => {
  ...
  const handleSendOTP = async (isResend = false) => {
    ...
    if (cooldownStart && Date.now() - cooldownStart < 10 * 60 * 1000) {
      setError('Please wait 10 minutes before trying again.');
      return;
    }

    if (otpSendCount >= 3) {
      setError('Too many attempts. Please try again in 10 minutes.');
      toast.error('🚫 Maximum OTP limit reached. Try again in 10 minutes.');
      return; // prevent timeout from running
    }
  ...
  <div className="relative flex-1">
    ...
    />
              ...
              <button
                ...
                disabled={loading || otpSendCount >= 3 || (cooldownStart && Date.now() - cooldownStart < 10 * 60 * 1000)}
              >
                {loading ? (
            ...
            <>
              ...
              <div className="mt-3 text-sm text-neutral-400 text-center">
                ...
                <button
                  ...
                  disabled={resendTimer > 0 || (cooldownStart && Date.now() - cooldownStart < 10 * 60 * 1000)}
