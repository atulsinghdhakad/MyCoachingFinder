...
const SettingsPage = () => {
  ...
  const { currentUser, logout, setUser, navigateTo, goBack, addToNavigationStack } = useAuth();
  ...
  useEffect(() => {
    ...
    console.log('🔧 SettingsPage: Auth context user:', currentUser);
    ...
    if (currentUser) {
      console.log('🔧 SettingsPage: User authenticated:', currentUser.email);
      setLoading(false);
  ...
  useEffect(() => {
    if (currentUser) {
      console.log('SettingsPage: currentUser.email =', currentUser.email);
      console.log('SettingsPage: isAdmin =', ADMIN_EMAILS.some(admin => admin.toLowerCase().trim() === (currentUser.email || '').toLowerCase().trim()));
    }
  }, [currentUser]);

  const isAdmin = currentUser && ADMIN_EMAILS.some(admin => admin.toLowerCase().trim() === (currentUser.email || '').toLowerCase().trim());
  ...
  useEffect(() => {
    if (currentUser?.uid) {
      ...
      if (currentUser.settings) {
        ...
        console.log('🔧 SettingsPage: Loaded settings from user context:', currentUser.settings);
      } else {
  ...
  }, [currentUser?.uid, currentUser?.settings]);

  // Load 2FA state from localStorage
  useEffect(() => {
    if (currentUser?.uid) {
      const twoFA = localStorage.getItem(`2fa_${currentUser.uid}`);
  ...
  useEffect(() => {
    if (
      ...
      currentUser.providerData.some((p) => p.providerId === 'google.com') &&
      !currentUser.providerData.some((p) => p.providerId === 'password') &&
      !localStorage.getItem(`password_linked_${currentUser.uid}`)
    ) {
  ...
  const handleSettingChange = (category, setting, value) => {
    ...
    if (currentUser?.uid) {
      localStorage.setItem(`settings_${currentUser.uid}`, JSON.stringify(newSettings));
      setUser(currentUser => ({
        ...currentUser,
      ...
      // Sync with backend
      fetch(`${process.env.REACT_APP_API_URL}/api/users/settings`, {
        method: 'POST',
        headers: {
  ...
  const handleEnable2FA = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        localStorage.setItem(`2fa_${currentUser.uid}`, (!twoFactorEnabled).toString());
  ...
  const handleDownloadData = async () => {
    ...
    try {
      const userData = {
        ...
        settings: settings,
        profilePicture: localStorage.getItem(`profile_${currentUser.uid}`) ? 'Stored locally' : 'Not found'
      };
  ...
  const handleSetPassword = async () => {
    try {
      ...
      if (newPassword) {
        ...
        localStorage.setItem(`password_linked_${currentUser.uid}`, 'true');
